...
export default function CrmModulePage({ moduleKey }: { moduleKey: string }) {
  ...
  return <section className="module-page">
    ...
    <div className="card data-card">
      ...
      <div className="tbl-wrap"><table className="tbl crm-table"><thead><tr>{page.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.join("-")} className="clickable">{row.map((value, cellIndex) => <td key={page.columns[cellIndex]} data-label={page.columns[cellIndex]}>{cellIndex === row.length - 1 ? <span className="badge st-ready">{value}</span> : cellIndex === 0 ? <b>{value}</b> : value}</td>)}</tr>)}</tbody></table></div>
      <div className="table-foot"><span>{rows.length} kayıt gösteriliyor</span><span>Demo verileri</span></div>
    </div>
    {modalOpen && <div className="modal-bg open" onMouseDown={() => setModalOpen(false)}><div className="modal crm-modal" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><p className="eyebrow">{page.eyebrow}</p><h3>{page.action}</h3></div><button className="icon-btn" onClick={() => setModalOpen(false)}><X size={18} /></button></div><div className="form-grid-demo"><ModuleForm moduleKey={moduleKey} /></div><div className="modal-actions"><button className="btn btn-ghost" onClick={() => setModalOpen(false)}>Vazgeç</button><button className="btn btn-primary" onClick={() => setModalOpen(false)}><Check size={15} /> {moduleKey === "ayarlar" ? "Ayarları kaydet" : "Demo kaydı oluştur"}</button></div></div></div>}
  </section>;
}

function Field({ label, children, wide = false }: { label: string; children: React.ReactNode; wide?: boolean }) {
  return <div className={`field${wide ? " wide" : ""}`}><label className="lbl">{label}</label>{children}</div>;
}

function BranchSelect() {
  return <select className="input"><option>Merkez Şube</option><option>Kadıköy Şubesi</option><option>Avrupa Yakası</option></select>;
}

function ModuleForm({ moduleKey }: { moduleKey: string }) {
  if (moduleKey === "musteriler") return <><Field label="Müşteri / firma adı"><input className="input" placeholder="Ad soyad veya firma" /></Field><Field label="Telefon"><input className="input" placeholder="05xx xxx xx xx" /></Field><Field label="E-posta"><input className="input" type="email" placeholder="ornek@firma.com" /></Field><Field label="Şube"><BranchSelect /></Field><Field label="Lead kaynağı"><select className="input"><option>Web sitesi</option><option>Referans</option><option>Sosyal medya</option><option>Telefon</option></select></Field><Field label="Sorumlu personel"><select className="input"><option>Mert Kaya</option><option>Ece Arslan</option><option>Selin Ak</option></select></Field><Field label="Müşteri notu" wide><textarea className="input" rows={3} placeholder="İlk görüşme notu..." /></Field></>;
  if (moduleKey === "islemler") return <><Field label="Müşteri"><select className="input"><option>Selin Yalçın</option><option>Aras Teknoloji</option><option>Mavi Yapı A.Ş.</option></select></Field><Field label="İşlem türü"><select className="input"><option>Randevu</option><option>Hizmet</option><option>Sipariş</option><option>Takip görüşmesi</option></select></Field><Field label="Şube"><BranchSelect /></Field><Field label="Tarih ve saat"><input className="input" type="datetime-local" /></Field><Field label="Sorumlu"><select className="input"><option>Mert Kaya</option><option>Ece Arslan</option><option>Selin Ak</option></select></Field><Field label="Tutar"><input className="input" type="number" placeholder="₺ 0,00" /></Field><Field label="İşlem notu" wide><textarea className="input" rows={3} placeholder="İşlem detayları..." /></Field></>;
  if (moduleKey === "subeler") return <><Field label="Şube adı"><input className="input" placeholder="Yeni şube adı" /></Field><Field label="Şube yöneticisi"><select className="input"><option>Yönetici seçin</option><option>Mert Kaya</option><option>Ece Arslan</option></select></Field><Field label="Telefon"><input className="input" placeholder="0212 xxx xx xx" /></Field><Field label="E-posta"><input className="input" type="email" placeholder="sube@firma.com" /></Field><Field label="Adres" wide><textarea className="input" rows={3} placeholder="Şube açık adresi..." /></Field></>;
  if (moduleKey === "personel") return <><Field label="Ad soyad"><input className="input" placeholder="Personel adı" /></Field><Field label="E-posta"><input className="input" type="email" placeholder="personel@firma.com" /></Field><Field label="Rol"><select className="input"><option>Satış Personeli</option><option>Operasyon</option><option>Finans Personeli</option><option>Şube Yöneticisi</option></select></Field><Field label="Şube"><BranchSelect /></Field><Field label="Telefon"><input className="input" placeholder="05xx xxx xx xx" /></Field><Field label="Başlangıç tarihi"><input className="input" type="date" /></Field></>;
  if (moduleKey === "hizmetler") return <><Field label="Ürün / hizmet adı"><input className="input" placeholder="Kayıt adı" /></Field><Field label="Tür"><select className="input"><option>Hizmet</option><option>Ürün</option><option>Paket</option><option>Abonelik</option></select></Field><Field label="Kategori"><select className="input"><option>Profesyonel Hizmet</option><option>Bakım</option><option>Teknik Hizmet</option><option>Destek</option></select></Field><Field label="Satış fiyatı"><input className="input" type="number" placeholder="₺ 0,00" /></Field><Field label="Sunulan şubeler" wide><select className="input"><option>Tüm şubeler</option><option>Merkez Şube</option><option>Kadıköy Şubesi</option><option>Avrupa Yakası</option></select></Field><Field label="Açıklama" wide><textarea className="input" rows={3} placeholder="Ürün veya hizmet açıklaması..." /></Field></>;
  if (moduleKey === "kasa") return <><Field label="Hareket türü"><select className="input"><option>Gelir / Tahsilat</option><option>Gider / Ödeme</option><option>Kasa transferi</option></select></Field><Field label="Şube"><BranchSelect /></Field><Field label="Cari / kategori"><input className="input" placeholder="Müşteri veya gider kategorisi" /></Field><Field label="Tutar"><input className="input" type="number" placeholder="₺ 0,00" /></Field><Field label="İşlem tarihi"><input className="input" type="date" /></Field><Field label="Ödeme yöntemi"><select className="input"><option>Nakit</option><option>Kredi kartı</option><option>Havale / EFT</option></select></Field><Field label="Açıklama" wide><textarea className="input" rows={3} placeholder="Kasa hareketi notu..." /></Field></>;
  if (moduleKey === "raporlar") return <><Field label="Rapor türü"><select className="input"><option>Şube performansı</option><option>Gelir-gider analizi</option><option>Müşteri dönüşümü</option><option>Personel performansı</option></select></Field><Field label="Şube"><select className="input"><option>Tüm şubeler</option><option>Merkez Şube</option><option>Kadıköy Şubesi</option></select></Field><Field label="Başlangıç"><input className="input" type="date" /></Field><Field label="Bitiş"><input className="input" type="date" /></Field><Field label="Çıktı biçimi" wide><select className="input"><option>Panel görünümü</option><option>PDF</option><option>Excel</option></select></Field></>;
  if (moduleKey === "yetkiler") return <><Field label="Rol adı"><input className="input" placeholder="Örn. Bölge Yöneticisi" /></Field><Field label="Şube kapsamı"><select className="input"><option>Atanan şubeler</option><option>Tüm şubeler</option><option>Yalnız merkez</option></select></Field><Field label="Modül erişimi" wide><div className="permission-grid"><label><input type="checkbox" defaultChecked /> Müşteriler</label><label><input type="checkbox" defaultChecked /> İşlemler</label><label><input type="checkbox" /> Personel</label><label><input type="checkbox" /> Finans</label><label><input type="checkbox" /> Raporlar</label><label><input type="checkbox" /> Ayarlar</label></div></Field></>;
  return <><Field label="İşletme adı"><input className="input" defaultValue="NovaCRM" /></Field><Field label="Varsayılan şube"><BranchSelect /></Field><Field label="Oturum süresi"><select className="input"><option>8 saat</option><option>4 saat</option><option>12 saat</option></select></Field><Field label="Bildirim tercihi"><select className="input"><option>E-posta + panel</option><option>Yalnız panel</option><option>Kapalı</option></select></Field></>;
}
